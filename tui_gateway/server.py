...
def _mirror_subagent_to_child(event_type: str, payload: dict) -> None:
    ...
    with _child_mirrors_lock:
        ...
        if not st["started"]:
            st["started"] = True
            _emit("message.start", csid, stream.start())
        if event_type == "subagent.thinking":
            ...
            if text := str(payload.get("text") or ""):
                _emit("message.delta", csid, stream.delta(text))
            ...
            if text := str(payload.get("text") or ""):
                _emit("message.delta", csid, stream.delta(f"{text}\n"))
            ...
            tool = {
                ...
                "tool_id": f"submirror:{child_key}:{st['tool_seq']}",
                "args": {},
            }
            ...
            _emit("message.complete", csid, stream.complete(summary))
            _child_mirrors.pop(child_key, None)
...
def _emit_terminal_turn_error(
    ...
    error: Any,
    *,
    stream: _TextStreamEnvelope | None = None,
    emit_complete=None,
) -> None:
    ...
    agent = session.get("agent")
    payload = {
        **(stream.complete(text) if stream is not None else {}),
        "text": text,
        ...
        try:
            ...
            try:
                ...
                _emit_terminal_turn_error(
                    ...
                    e,
                    stream=stream,
                    emit_complete=_emit_stream_completion,
                )
                turn_error_retained = True
