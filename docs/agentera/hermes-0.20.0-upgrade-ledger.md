...
| `L5-BROAD-RED-001` | `753d486ea5d6175c86cc549af7a289bac39b705a` | single allowed `scripts/run_tests.sh` broad local gate | macOS arm64, 30 workers, canonical runner and clean tracked worktree | failed | 2,618 files: 25,628 tests passed and 61 failed in 1,393.8s. Five failures were stale direct-owner test expectations fixed below; the remaining failures span 29 unrelated upstream cross-platform/optional-dependency files, plus two isolated `agent-browser install` timeouts killed by the runner at 300s. This gate is retained as failed and is not rerun unchanged. |
| `L5-EXPANDED-OWNERS-GREEN-001` | `753d486e` + direct-test diff `bf4ab7aca5280f55ec20344b3a7c051e230d7c1d96ca61fd3466125a05813b2e` | `scripts/run_tests.sh tests/hermes_cli/test_models.py tests/hermes_cli/test_ensure_hermes_home_memo.py tests/tui_gateway/test_auto_continue.py tests/tui_gateway/test_kanban_notify_poller.py -q` | local shared development venv, canonical test runner, changed direct-owner input | passed | 4 files, 63 passed, 0 failed. Tests now match upstream's Qwen 3.8 catalog, Aera's deliberate per-call home repair, and the single sequenced `message.start` emitted by `_run_prompt_submit`. |
